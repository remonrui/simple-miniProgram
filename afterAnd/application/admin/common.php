...
function get_nickname($uid = 0){
    ...
    /* 获取缓存数据 */
    if(empty($list)){
        $list = \think\Cache::get('admin_user_nickname_list');
    }
    ...
    if(isset($list[$key])){ //已缓存，直接使用
        ...
        $info = \app\common\model\Member::get($uid);
        if($info !== false && $info['nickname'] ){
            ...
            \think\Cache::set('admin_user_nickname_list', $list);
        } else {
            $name = '';
        }
    }
...
function get_category($id, $field = null){
    ...
    /* 读取缓存数据 */
    if(empty($list)){
        $list = S('admin_category_list');
    }

    /* 获取分类名称 */
    if(!isset($list[$id])){
        $cate = think\Db::name('Category')->find($id);
        if(!$cate || 1 != $cate['status']){ //不存在分类，或分类被禁用
            return '';
        }
        $list[$id] = $cate;
        S('admin_category_list', $list); //更新缓存
    }
...
function get_top_model($model_id=null){
    ...
    $model = think\Db::name('Model')->where($map)->field(true)->select();
...
function action_log($action = null, $model = null, $record_id = null, $user_id = null){
    ...
    think\Db::name('ActionLog')->add($data);
...
function parse_action($action = null, $self){
    ...
    //查询行为信息
    $info = think\Db::name('Action')->where($map)->find();
...
function execute_action($rules = false, $action_id = null, $user_id = null){
    ...
    foreach ($rules as $rule){
        ...
        $exec_count = think\Db::name('ActionLog')->where($map)->count();
        if($exec_count > $rule['max']){
            continue;
        }

        //执行数据库操作
        $Model = think\Db::name(ucfirst($rule['table']));
        $field = $rule['field'];
...
function get_model_attribute($model_id, $group = true,$fields=true){
    ...
    if(!isset($list[$model_id])){
        ...
        $extend = think\Db::name('Model')->getFieldById($model_id,'extend');

        if($extend){
            $map = array('model_id'=> array("in", array($model_id, $extend)));
        }
        $info = think\Db::name('Attribute')->where($map)->field($fields)->select();
        $list[$model_id] = $info;
    }
    ...
    if($group){
        ...
        $model     = think\Db::name("Model")->field("field_sort,attribute_list,attribute_alias")->find($model_id);
 ...
 * api('Center/User/getName','id=5');  调用Admin模块的User接口
...
function get_table_field($value = null, $condition = 'id', $field = null, $table = null){
    ...
    //拼接参数
    $map[$condition] = $value;
    $info = think\Db::name(ucfirst($table))->where($map);
    if(empty($field)){
...
function get_link($link_id = null, $field = 'url'){
    ...
    $link = think\Db::name('Url')->getById($link_id);
    if(empty($field)){
        return $link;
    }else{
        return $link[$field];
